Prerequisites
1. Need Azure subscription and an Azure Event Hub created
2. Maven and Java installed in the development machine

For development in Eclipse, 
1. Open Eclipse in a new workspace
2. import 'Existing Maven Project'
3. select the folder, 'TBCodeEvents' (from this codebase), where you see the pom.xml file 

In order to build and run the standalone java program, run the following, from the TBCodeEvents folder.
1. Make sure you create the Azure Event Hub and place the properties from it to the eventhub.properties file here.
2. From command line in the TBCodeEvents folder, run 
	> mvn install
3. From command line in the TBCodeEvents folder, run 
	> mvn package


If you are sending data to Event Hub run
> java -jar .\target\TBCodeEvents-0.0.1-SNAPSHOT-jar-with-dependencies.jar META-INF/eventhub.properties send

If you are receiving data from Event Hub, run
> java -jar .\target\TBCodeEvents-0.0.1-SNAPSHOT-jar-with-dependencies.jar META-INF/eventhub.properties receive
